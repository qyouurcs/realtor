    ...
    for root, subdirs, fns in os.walk(fn_path_dir):
        for fn in fns:
            ...
            for batch_idx in xrange(batch_num):
                ...
                area_code = fn.split('_')[1]
                if not os.path.isdir(os.path.join(save_dir, area_code)):
                    os.makedirs(os.path.join(save_dir, area_code))
                batch_fn = os.path.join(save_dir,area_code, 'batch-{0}'.format(batch_idx))

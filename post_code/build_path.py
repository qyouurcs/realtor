...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print 'Usage: {0} <sim_fn> <san-jose-420-price-filtered-area-code_train.txt> [avg num_path/node=2] [len_p=10]'.format(sys.argv[0])
        sys.exit()
    ...
    if len(sys.argv) >= 4:
        num_p_node = int(sys.argv[3])
    if len(sys.argv) >= 5:
        len_p = int(sys.argv[4])

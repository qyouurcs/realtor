...
if __name__== '__main__':
    ...
    if len(sys.argv) < 5:
        print 'Usage: {0} <lat_long.txt> <san-jose-420-price-filtered-area-code_train.txt> <...val.txt> <save_dir> [ dist = 5 miles]'.format(sys.argv[0])
        sys.exit()
    ...
    for code in dict_code:
        save_fn = os.path.join(save_dir, 'dist_{0}_{1}.txt'.format(code, dist))
        with open(save_fn, 'w') as fid:
